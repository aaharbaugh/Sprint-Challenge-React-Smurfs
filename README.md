...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
	...
	server side routing is, every time a new url is accessed, a new request must be made to the server that responds with all required files. this is not always good because it can be bandwidth heavy for both the server and the client. 

	client side routing, the first time the client accesses the server, it recives all information and then that information can route and render the clients information. making this faster and less bandwidth intensive. 

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

	Get request is equivalent to a Read. 
	Post request is equivalent to Create
	Put request is equivalent to Update
	Delete request is equivalent to Destroy. 


- [ ] Mention three tools we can use to make AJAX requests.

	Javascripts Fetch method. 
	Axios Library. 
	jquery
